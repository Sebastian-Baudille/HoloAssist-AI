"""
extend_env.py — Sub-policy 2: extend arm to reach the cube.

Assumes pan is already correct (shoulder_pan locked at optimal angle for cube).
Policy controls lift + elbow to position EE directly above/at cube.

Obs (5D):    [xy_dist_norm, z_err_norm, lift_norm, elbow_norm, wrist1_norm]
Action (3D): [Δlift, Δelbow, Δwrist_1] × 0.05 rad/step
Fixed:       pan=optimal, wrist_2=-π/2, wrist_3=0

Reward: -dist + progress*2   (always negative, success is only escape)
...
_FIXED_W2 = -np.pi / 2
_FIXED_W3 = np.pi / 2
FLOOR_Z_IDEAL = 0.075  # ideal approach height — EE rewarded for staying here

# Pan's end-state pose — extend always resets from here
_HOME_LIFT  = -np.pi / 2
_HOME_ELBOW = 1.7
_RESET_NOISE = 0.15   # ±rad noise around home so policy sees variety
...
class UR3eExtendEnv(gym.Env):
    ...
    def _obs(self, joints, ee_pos, cube_pos):
        ...
        z_err    = float(ee_pos[2] - cube_pos[2])  # positive = too high
        return np.array([
    ...
    def step(self, action):
        ...
        q = np.array([self._locked_pan, new_lift, new_elbow,
                      new_w1, _FIXED_W2, _FIXED_W3])
        self.data.ctrl[:6] = q
        ...
        xy_dist  = float(np.linalg.norm(ee_pos[:2] - cube_pos[:2]))
        progress = self._prev_dist - xy_dist
        self._prev_dist = xy_dist

        z_err_ideal  = float(ee_pos[2]) - FLOOR_Z_IDEAL
        height_reward = -2.0 * abs(z_err_ideal)   # zero at ideal height, penalises above and below

        success    = xy_dist < GRASP_DIST_M
        reward     = -2.0 * (xy_dist / 0.50) + progress * 3.0 + height_reward + (50.0 if success else 0.0)
        terminated = success
        ...
        return (self._obs(joints, ee_pos, cube_pos), reward,
                ...
                {"is_success": success, "dist_to_cube": xy_dist})
    ...
    def reset(self, seed=None, options=None):
        ...
        # Start from pan's end-state with small noise
        ...
        # FK check — keep EE above floor
        start = np.array([self._locked_pan, lift, elbow, -np.pi/2, _FIXED_W2, _FIXED_W3])
        if forward_kinematics(start)[2] < 0.02:
        ...
        cube_pos = self._get_cube_pos()
        joints   = self._get_joints()
        self._prev_dist = float(np.linalg.norm(ee_pos[:2] - cube_pos[:2]))
