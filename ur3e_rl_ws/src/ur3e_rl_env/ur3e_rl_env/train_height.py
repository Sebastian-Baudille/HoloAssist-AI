"""Train Sub-policy 3: Fine-tune wrist_1 for height/Z alignment."""
...
from ur3e_rl_env.envs.height_env import UR3eHeightEnv

_REPO     = Path(__file__).parent.parent.parent.parent.parent
MODEL_DIR = _REPO / "ur3e_rl_ws" / "rl_models" / "height"
LOG_DIR   = _REPO / "ur3e_rl_ws" / "height_tb_logs"
...
def main():
    ...
    callbacks = [
        CheckpointCallback(save_freq=max(5_000//args.envs, 1),
                           save_path=str(CKPT_DIR), name_prefix="height"),
        EvalCallback(eval_env, best_model_save_path=str(BEST_DIR),
                     eval_freq=max(10_000//args.envs, 1), n_eval_episodes=20, verbose=1),
    ]
    ...
    print(f"\nTraining Height model — {args.timesteps:,} steps, {args.envs} envs")
    try:
        model.learn(args.timesteps, callback=callbacks, reset_num_timesteps=not args.load,
                    progress_bar=True)
        model.save(str(MODEL_DIR / "height_final"))
        print(f"Saved → {MODEL_DIR / 'height_final'}.zip")
    finally:
        train_env.close()
        eval_env.close()
