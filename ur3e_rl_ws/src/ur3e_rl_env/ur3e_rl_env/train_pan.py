"""Train Sub-policy 1: Pan to face the cube."""
...
from ur3e_rl_env.envs.pan_env import UR3ePanEnv

_REPO     = Path(__file__).parent.parent.parent.parent.parent
MODEL_DIR = _REPO / "ur3e_rl_ws" / "rl_models" / "pan"
LOG_DIR   = _REPO / "ur3e_rl_ws" / "pan_tb_logs"
...
def main():
    ...
    callbacks = [
        CheckpointCallback(save_freq=max(5_000//args.envs, 1),
                           save_path=str(CKPT_DIR), name_prefix="pan"),
    ...
    print(f"\nTraining Pan model — {args.timesteps:,} steps, {args.envs} envs")
    try:
        model.learn(args.timesteps, callback=callbacks, reset_num_timesteps=not args.load,
                    progress_bar=True)
        model.save(str(MODEL_DIR / "pan_final"))
        print(f"Saved → {MODEL_DIR / 'pan_final'}.zip")
    finally:
        train_env.close()
        eval_env.close()
