...
class SceneController(Node):
    def __init__(self):
        ...
        self.declare_parameter("cube_size_min",   0.04)
        self.declare_parameter("cube_size_max",   0.04)
        ...
        # Track whether we've already removed the static SDF cubes (cube_red etc.)
        # that are baked into the world file at launch. Only needed once per session.
        self._sdf_defaults_cleared = False
    ...
    def _on_randomize(self, request, response):
        try:
            ...
            # First call only: remove the static cubes baked into the world SDF
            # (cube_red, cube_green, etc.) — _remove_tracked() doesn't know about them.
            if not self._sdf_defaults_cleared:
                self._remove_sdf_defaults()
                self._sdf_defaults_cleared = True
    ...
    def _remove_sdf_defaults(self):
        """Remove the static cubes baked into the world SDF at launch time.

        These are named cube_red/green/blue/yellow in sim_params.yaml and are
        spawned by Gazebo automatically — scene_controller never tracks them,
        so _remove_tracked() leaves them behind. Call once per session.
        """
        import time
        world = self.get_parameter("world_name").value
        names = [c["name"] for c in self._default_cubes]
        for name in names:
            subprocess.run([
        ...
        if names:
            self.get_logger().info(f"Removed SDF default cubes: {names}")
            time.sleep(0.3)
